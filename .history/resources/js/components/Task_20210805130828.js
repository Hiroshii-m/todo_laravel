...
export default class Task extends React.Component {
    ...
    render() {
        ...
        // todosの中身があれば、それぞれ、「分」のデータを「時間：分」に直す
        if(Object.keys(todos).length !== 0) {
            console.log('空じゃないです');
            for(let i in todos) {
                console.log(todos[i].expect_time);
                console.log(todos[i].spend_time);
                todos[i]['expHour'] = Math.floor(todos[i].expect_time/60);
                todos[i]['expMin'] = todos[i].expect_time - (todos[i]['expHour'] * 60);
                todos[i]['speHour'] = Math.floor(todos[i].spend_time/60);
                todos[i]['speMin'] = todos[i].spend_time - (todos[i]['speHour'] * 60);
            }
        }else{
            console.log('空です');
        }
