...
export default class TaskList extends React.Component {
    ...
    render() {
        ...
        // 整数部分を時間、小数部分を分として整理する
        expTimeTotal = Math.floor(expMinuteTotal/60);
        expMinuteTotal = Math.ceil(parseFloat("0." + (String(expMinuteTotal/60).split(".")[1]) * 60));
        // 実行時間の合計を時間と分に直す。
        speMinuteTotal = (speTimeTotal * 60) + speMinuteTotal;
        speTimeTotal = Math.floor(speMinuteTotal/60);
        speMinuteTotal = Math.ceil(parseFloat("0." + (String(speMinuteTotal/60).split(".")[1]) * 60));
