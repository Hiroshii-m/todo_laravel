...
export default class Task extends React.Component {
    ...
    render() {
        ...
        const expTime = (Number.isInteger(this.state.expTime)) ? this.state.expTime : 0;
        const expMinute = (Number.isInteger(this.state.expMinute)) ? this.state.expMinute : 0;
        const speTime = (Number.isInteger(this.state.speTime)) ? this.state.speTime : 0;
        const speMinute = (Number.isInteger(this.state.speMinute)) ? this.state.speMinute : 0;
        // Taskの状態から要素を分ける
                            ...
                            <div className="c-acd__time">
                                <div className="">予想時間：
                                    <div className="p-todoList__time u-flex">
                                        <input className="p-todoList__timeInput" min="0" type="number" value={expTime} onChange={this.handleChangeExpTime} />:<input className="p-todoList__timeInput" type="number" min="0" max="59" value={expMinute} onChange={this.handleChangeExpMinute} />
                                    </div>
                                </div>
                                <div>作業時間：
                                    <div className="p-todoList__time u-flex">
                                        <input className="p-todoList__timeInput" min="0" type="number" value={speTime} onChange={this.handleChangeSpeTime} />:<input className="p-todoList__timeInput" type="number" min="0" max="59" value={speMinute} onChange={this.handleChangeSpeMinute} />
                                    </div>
                                </div>
                                {timer}
