                    ...
                    <div class="p-boardList__left">
                        ...
                        <ul class="p-boardList__optionMenu c-itemMenu js-target-display">
                            <li class="p-boardList__optionItem c-itemMenu__item">
                                <a href="{{ route('clusters', $board->id) }}" class="c-itemMenu__txt"><i class="far fa-folder-open u-margin--0-m"></i>{{ __('Open') }}</a>
                            </li>
                            <form action="{{ route('boards.delete', $board->id) }}" method="POST" class="p-boardList__optionItem c-itemMenu__item">
                                @method('DELETE')
                                @csrf
                                <button class="c-itemMenu__txt" onclick='return confirm("削除しますか？");'><i class="fas fa-trash-alt u-margin--0-m"></i>{{ __('Delete') }}</button>
                            </form>
                        </ul>
                    </div>
                </li>
