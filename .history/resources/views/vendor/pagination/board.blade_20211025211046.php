@if ($paginator->hasPages())
    <ul class="pagination c-pagi p-boardPage" role="navigation">
        ...
        @if ($paginator->onFirstPage())
            <li class="disabled c-pagi__item p-boardPage" aria-disabled="true" aria-label="@lang('pagination.previous')">
                <span aria-hidden="true"><i class="fas fa-chevron-left"></i></span>
            </li>
        @else
            <li class=" c-pagi__item p-boardPage">
        ...
        @foreach ($elements as $element)
            ...
            @if (is_string($element))
                <li class="disabled c-pagi__item p-boardPage" aria-disabled="true"><span>{{ $element }}</span></li>
            @endif
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="active c-pagi__item p-boardPage" aria-current="page"><span>{{ $page }}</span></li>
                    @else
