...
<section id="l-main">
    <form class="c-form" method="POST" action="{{ route('login') }}">
        ...
        <label for="remember" class="c-form__label">
            {{ __('Password') }}
            <input id="password" type="text" class="c-form__input" name="remember" value="{{ old('password') }}" required autocomplete="new-password" autofocus>
            @error('password')
                <p class="c-form__error">{{ $message }}</p>
            @enderror
        </label>
