...
@section('content')
<!-- form -->
<form class="p-form u-margin--0-auto" method="POST" action="{{ route('login') }}">
    @csrf
    ...
    <label for="remember">
        {{ __('Remember Me') }}
        <input class="p-form__check u-inline" type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}>
    </label>
    
    <button class="p-form__submit u-bgColor--primary">{{ __('Login') }}</button>
